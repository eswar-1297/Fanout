...
type AIProvider = 'chatgpt' | 'gemini' | 'perplexity' | 'all';
...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use relative path - works in both dev (proxy) and production (same server)
        ...
        {result && (
          <div className="results">
            ...
            <div className="results-meta">
              ...
              {result.used_providers && result.used_providers.length > 0 && (
                <span><strong>Generated by:</strong> {result.used_providers.join(', ')}</span>
              )}
            </div>
